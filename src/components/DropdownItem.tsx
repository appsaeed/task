...
export default function DropdownItem({ link, name = 'name', ...props }: DropdownItemProps) {
    return (
        <li onClick={() => alert('go')} {...props} className={cn("block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 dark:text-gray-200 dark:hover:text-white", props.className)}>
            {
                link
                    ?
